...
public class ListCashController implements Initializable
{
	...
	private void mLoadData(List<CashDTO> cashs)
	{
		this.cashs = cashs;

		int[] ids = new int[cashs.size()];
		List<List<String>> finalList = new ArrayList<>();

		int rowNo = 1;
		for (CashDTO cash : cashs)
		{

			ids[rowNo - 1] = cash.getId();

			List<String> l = new ArrayList<>();
			l.add(String.valueOf(rowNo));
			l.add(String.valueOf(cash.getId()));
			l.add(cash.getName());
			l.add(cash.getNote());
			finalList.add(l);
			rowNo++;
	...
	private void handleClickBtnDelete(ActionEvent event)
	{
		...
		else
		{
			...
			{
				CashSer objCashSer = new CashSer();

				objCashSer.delete(dotNetTable.getSelectedId());
				handleClickBtnSearch();
	...
	{
		this.mLoadData(new CashSer().retrieve());
	}
	...
	private void handleClickBtnAdd(ActionEvent event)
	{
		...
		FXMLLoader fxmll = StageHelper.getFXMLLoader(FXMLLayouts.ADD_CASH);
		AddCashController addCashController = fxmll.getController();
	...
	private void handleClickBtnEdit(ActionEvent event)
	{
		...
		FXMLLoader fxmll = StageHelper.getFXMLLoader(FXMLLayouts.ADD_CASH);
		AddCashController addCashController = fxmll.getController();
		addCashController.setData(cashSer.getById(dotNetTable.getSelectedId()));
		Container.wrapperController.addPane(fxmll);
